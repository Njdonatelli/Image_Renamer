...
# Show current state
Write-Host "[1] Current files in Analyze_Images/`n"
Get-ChildItem Analyze_Images -File | ForEach-Object { Write-Host "    $($_.Name)" }
...
sample_dir = Path('Analyze_Images')
...
Write-Host "[3] Result - Files in Analyze_Images/`n"
Get-ChildItem Analyze_Images -File | ForEach-Object { Write-Host "    $($_.Name)" }
