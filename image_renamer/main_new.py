...
from image_renamer.analyzer import GeminiCliAnalyzer, GeminiSDKAnalyzer
...
class ImageRenamerCLI:
    """Command-line interface for batch image renaming."""
    
    def __init__(self):
        """Initialize CLI with analyzer and filename generator."""
        # Try CLI first, fall back to SDK
        self.analyzer = GeminiCliAnalyzer()
        
        if not self.analyzer.gemini_available:
            logger.warning("Gemini CLI not available, attempting to use Python SDK (Gemini 2.5-Flash)")
            self.analyzer = GeminiSDKAnalyzer()
            if not self.analyzer.client:
                logger.error("Neither Gemini CLI nor Python SDK is available.")
                logger.info("Install Gemini CLI: https://ai.google.dev/gemini-cli")
                logger.info("Or install Python SDK: pip install google-generativeai")
                sys.exit(1)
